...
VertRes::Pipelines::RNA_Seq - pipeline for processing mRNA Sequencing data from BAM files

=head1 SYNOPSIS

Takes RNA Seq mapping from BAM files and calculates readcount, RPKM and FKPM for each gene.

Input: BAM and GFF files.
Output: Tab delimited files listing readcount, RPKM and FKPM for each gene in GFF file.

Work in progress. 
Using SNP pipeline as template. 
...
package VertRes::Pipelines::RNA_Seq;
...
use VertRes::Wrapper::GATK; # Not required
use File::Copy;

# Note Proposed actions: 
# - Filter to produce new bam file - ignore map == 0 (use min cutoff default to zero.)
